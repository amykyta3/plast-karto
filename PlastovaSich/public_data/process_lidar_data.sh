...
function dl_laz {
    ...
    wget -nc -nv -O $1 ${root_url}/$1

    if [ $? -eq 0 ]; then
        $LAStools/laszip -i $1 -o ${1%.laz}.las
    fi
    set -e
}
...
$LAStools/las2las -keep_xy 737000 4872500 738400 4874200 -i tmp/merged.las -o tmp/cropped.las
$LAStools/las2las -keep_xy 737010 4872500 738400 4874200 -i tmp/merged.las -o tmp/cropped-shifted.las
...
wine $LAStools/las2iso -cpu64 -i tmp/elev-m.las -o processed/iso-0.5m.shp \
    -iso_every 0.5 -smooth 4 -clean 30 -simplify 1
